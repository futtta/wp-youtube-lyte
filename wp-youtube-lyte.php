...
function lyte_parse($the_content,$doExcerpt=false) {
    ...
    if ( get_option('lyte_greedy','1')==="1" && strpos($the_content,"youtu") !== false ){
        ...
        if ( apply_filters( 'lyte_eats_yframes', true ) && preg_match_all('#<iframe(?:.*)?\ssrc=["|\']https:\/\/www\.youtube(?:-nocookie)?\.com\/embed\/(.*)["|\']\s(?:.*)><\/iframe>#Usm', $the_content, $matches, PREG_SET_ORDER)) {
